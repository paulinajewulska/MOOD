...
class MovieRatingService:
    def __init__(self, movie_rating_repository):
        if not isinstance(movie_rating_repository, MovieRatingRepository):
            raise Exception(
                "movie_rating_repository is not instance of MovieRatingRepository")
    ...
    def add_movie_to_rating(self, user_id, movie_id):
        movie_to_rating = MovieWithRating(
            user_id=user_id, movie_id=movie_id, rating=MovieRating.WAITING_FOR_RATING)
